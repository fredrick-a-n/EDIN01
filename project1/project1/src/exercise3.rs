...
pub fn exercise3() {
    ...
    let num: BigUint = BigUint::parse_bytes(b"127423368713324519534591", 10).unwrap();

    if let Some((f1, f2)) = find_factors(num.clone()) {
        log::info!("Two factors of {num} are {f1} and {f2}", num = num, f1 = f1, f2 = f2);
    } else {
    ...
    for solution_vec in find_solutions(matrix) {
        ...
        let b = r_vec
            ...
            .map(|((_, p_vec), solution)| {
                p_vec.iter().map(|x| *solution as u32 * (*x) as u32).collect::<Vec<u32>>()
            })
            .reduce(|x: Vec<u32>, y: Vec<u32>| {
...
fn recursive_gauss(mut left: ndarray::ArrayViewMut2<i8>, mut right: ndarray::ArrayViewMut2<i8>) {
    ...
    let pos = left.axis_iter_mut(ndarray::Axis(0)).enumerate().find(|(_ ,row)| row[0] == 1);
